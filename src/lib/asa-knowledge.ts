...
export function getAsaKnowledgeBase(): AsaKnowledgeEntry[] {
  ...
  if (hasSisterSchoolLink()) {
    entries.push({
      id: "sister-campus",
      keywords: ["thembisa", "other campus", "sister school", "second campus", "other site"],
      buildAnswer: () =>
        `We also have **${SISTER_SCHOOL_LINK.label}** — a separate campus site you can open from the menu. This Pretoria site covers **${SCHOOL_INFO.suburb}**.`,
